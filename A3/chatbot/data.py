    ...
    for fragment in line.strip().lower().split():
        for token in re.split(_WORD_SPLIT, fragment):
            if not token:
                continue
            if normalize_digits:
                token = re.sub(_DIGIT_RE, '#', token.decode(errors='ignore'))
            words.append(token.encode())
    return words
        ...
        for word in sorted_vocab:
            if vocab[word] < config.THRESHOLD:
                with open('config.py', 'ab') as cf:
                    if filename[-3:] == 'enc':
                        if not hasattr(config, 'ENC_VOCAB') or config.ENC_VOCAB != index:
                            cf.write(b'ENC_VOCAB = ' + str(index).encode() + b'\n')
                    else:
                        if not hasattr(config, 'DEC_VOCAB') or config.DEC_VOCAB != index:
                            cf.write(b'DEC_VOCAB = ' + str(index).encode() + b'\n')
                break
            f.write(word + b'\n')
            index += 1
...
def token2id(data, mode):
    ...
    for line in lines:
        ...
        out_file.write(b' '.join(str(id_).encode() for id_ in ids) + b'\n')
